...
let CareerHighlightsId: string | null = null;

async function initializeGoogleDrive() {
    if (driveClient && CareerHighlightsId) {
        return;
    }
    ...
    if (process.env.GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID) {
        CareerHighlightsId = process.env.GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID;
    } else {
        console.warn('GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID not set. Attempting to find "Career Highlights" folder by name. For production, hardcode the ID or set the env var.');
        try {
            const res = await driveClient.files.list({
                q: "name='Career Highlights' and mimeType='application/vnd.google-apps.folder'",
                fields: 'files(id, name)',
                spaces: 'drive',
            });
            const folder = res.data.files?.[0];
            if (folder && folder.id) {
                CareerHighlightsId = folder.id;
                console.log(`Found "Career Highlights" folder with ID: ${CareerHighlightsId}`);
            } else {
                throw new Error('Google Drive folder "Career Highlights" not found. Please create it and/or set GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID.');
            }
        } catch (error) {
...
export async function readAboutEdwin(topicName: string): Promise<string> {
    ...
    try {
        const res = await driveClient.files.list({
            q: `'${CareerHighlightsId}' in parents and name='${topicName}' and mimeType='application/vnd.google-apps.document'`,
            fields: 'files(id, name)',
            pageSize: 1,
        });
...
export const getTopicNames = async (): Promise<string[]> => {
    ...
    try {
        const res = await driveClient.files.list({
            q: `'${CareerHighlightsId}' in parents and mimeType='application/vnd.google-apps.document'`,
            fields: 'files(name)',
            pageSize: 100,
        });
